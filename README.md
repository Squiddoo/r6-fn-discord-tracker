...
To **send current stats to Discord once** (even if nothing changed): Run workflow, check **Send a one-time Discord preview**, optionally pick Siege / Fortnite / both, then run. That posts one webhook message per game with every fetched player, marked as Preview. The 10-minute job still only posts on real changes.
...
One-time Discord preview of current stats (does not wait for a change):
